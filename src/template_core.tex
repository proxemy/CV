...
\newcommand{\IfNonEmptyElse}[2]{\ifthenelse{\equal{#1}{}}{#2}{#1}}
...
\newcommand{\CvItem}[5]{ % adds CV rows of work places into a table
	...
	#1 – #2 &
	\ifthenelse{\equal{#4#5}{}}{#3}{\textbf{#3}} % optional bold company (CV item) name
	\IfNonEmpty{#4}{– #4} % optional job title#4
	\IfNonEmpty{#5}{\begin{addmargin}[1em]{1em}#5\end{addmargin}} % optional job/task-descriptions#5
	\\}
...
\def\RecpSalutation{%
	\IfNonEmpty{\RecpCompany}{%
		Sehr geehrte Damen und Herren \RecpCompanySalutForm{}
			\IfNonEmptyElse{\RecpCompanySpecialSalut}{\RecpCompany},
		\IfNonEmpty{\RecpName}{\newline}
	}
		\IfNonEmpty{\RecpName}{%
